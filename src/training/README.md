...
| `losses.py` | `SafetyAwareAsymmetricHuberLoss` (SAHL), `WeightedMSELoss`, and `build_loss` factory |
...
Key options:
- `--loss_type`: `smoothl1` (symmetric Huber), `mse`, `weighted_l1`, `weighted_mse`, or `sahl` (new SAHL with optional Huber beta)
