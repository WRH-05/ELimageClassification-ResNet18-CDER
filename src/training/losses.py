...
Example:
    ...
    ...     threshold=0.66, critical_weight=2.5, beta=0.5
    ... )
...
class SafetyAwareAsymmetricHuberLoss(nn.Module):
    ...
    Samples whose target severity is at or above ``threshold`` receive a
    ``critical_weight`` multiplier, directing the model to prioritise
    high-severity defect regions.
    ...
    **Backward compatibility**: When ``beta=0.0`` (default), the loss
    degenerates to a purely asymmetric L1 loss, producing identical
    behaviour to the original ``WeightedL1Loss`` used in earlier
    experiments.
    ...
    .. math::
        ...
        w &=
        \\begin{cases}
            w_{\\text{critical}} & \\text{if } y \\geq \\tau \\\\
            w_{\\text{normal}}   & \\text{otherwise}
        \\end{cases} \\\\[4pt]
        H(\\delta) &=
        \\begin{cases}
    ...
    Attributes:
        threshold: Target value above which a sample is safety-critical.
        critical_weight: Loss multiplier for safety-critical samples.
        normal_weight: Loss multiplier for normal samples.
        beta: Huber transition parameter (0.0 → pure L1).

    Args:
        threshold: Target severity value at or above which samples are
            considered safety-critical (default: ``0.66``).
        critical_weight: Loss multiplier for safety-critical samples
            (default: ``2.5``).
    ...
    def forward(
        ...
        # --- Safety-aware asymmetric weighting ---
        weights = torch.where(
